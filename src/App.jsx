...
/* ============================================================
   AFINADOR — GuitarTuner
   Usa Web Audio API (AnalyserNode) + algoritmo YIN simplificado
   para detectar o pitch do microfone em tempo real.
   Visual inspirado em afinadores de referência: mostrador circular,
   ponteiro animado, corda selecionada, status "OK / ♭ / ♯".
   ============================================================ */
function GuitarTuner() {
  const [active, setActive]         = useState(false);
  const [note, setNote]             = useState(null);        // { name, octave, freq, cents, semitone }
  const [selString, setSelString]   = useState(null);        // string selecionada manualmente (null = auto)
  ...
  const GUITAR_STRINGS = [
    { label: "E2", freq: 82.41,  midi: 40, display: "E" },
    { label: "A2", freq: 110.00, midi: 45, display: "A" },
    { label: "D3", freq: 146.83, midi: 50, display: "D" },
    { label: "G3", freq: 196.00, midi: 55, display: "G" },
    { label: "B3", freq: 246.94, midi: 59, display: "B" },
    { label: "E4", freq: 329.63, midi: 64, display: "E" },
  ];
  const BASS_STRINGS = [
    { label: "B1", freq: 30.87,  midi: 23, display: "B" },
    { label: "E2", freq: 41.20,  midi: 28, display: "E" },
    { label: "A2", freq: 55.00,  midi: 33, display: "A" },
    { label: "D3", freq: 73.42,  midi: 38, display: "D" },
    { label: "G3", freq: 98.00,  midi: 43, display: "G" },
  ];
  ...
  function freqToNote(freq) {
    ...
    return { name, octave, freq, cents: Math.round(cents), semitone: round };
  }
  ...
  function detectPitch(buf, sampleRate) {
    ...
    // Diferença normalizada cumulativa
    const cmn = new Float32Array(HALF);
    cmn[0] = 1;
    let runSum = 0;
    for (let tau = 1; tau < HALF; tau++) {
      runSum += diff[tau];
      cmn[tau] = runSum === 0 ? 0 : diff[tau] * tau / runSum;
    }

    // Acha o primeiro vale abaixo do threshold
    let tau = 2;
    while (tau < HALF) {
      if (cmn[tau] < thresh) {
    ...
    if (tau === HALF || cmn[tau] >= thresh) return null;

    // Interpolação parabólica para mais precisão
    const x0 = tau > 1    ? tau - 1 : tau;
    const x2 = tau + 1 < HALF ? tau + 1 : tau;
    if (x0 === tau) return sampleRate / (cmn[tau] <= cmn[x2] ? tau : x2);
    if (x2 === tau) return sampleRate / (cmn[tau] <= cmn[x0] ? tau : x0);
    const s0 = cmn[x0], s2 = cmn[x2];
    const betterTau = tau + (s2 - s0) / (2 * (2 * cmn[tau] - s2 - s0));
  ...
  async function toggle() {
    ...
    try {
      ...
      setActive(true);
      // Aguarda o próximo frame para garantir que o estado "active" esteja definido
  ...
  // ── Calcula desvio em cents vs. corda selecionada ────────
  function getCentsVsString(note) {
    if (!note) return null;
    const target = selString != null ? strings[selString] : null;
    if (!target) return note.cents; // usa cents vs. nota mais próxima
    const diffMidi = 12 * Math.log2(note.freq / target.freq);
    return Math.round(diffMidi * 100);
  }

  // ── Detecta qual corda é mais próxima (auto) ────────────
  function getNearestString(note) {
    if (!note) return null;
    let best = null, bestDiff = Infinity;
    strings.forEach((s, i) => {
      const diff = Math.abs(12 * Math.log2(note.freq / s.freq));
      if (diff < bestDiff) { bestDiff = diff; best = i; }
    });
    return best;
  }

  const cents        = note ? getCentsVsString(note) : 0;
  const clampedCents = Math.max(-60, Math.min(60, cents || 0));
  const nearestIdx   = note ? getNearestString(note) : null;
  const activeIdx    = selString != null ? selString : nearestIdx;

  // Ponteiro: mapa de –60…+60 cents → –80°…+80°
  const angle = (clampedCents / 60) * 80;

  const isInTune   = note && Math.abs(cents) <= 5;
  const isFlat     = note && cents < -5;
  const isSharp    = note && cents > 5;

  const needleColor  = isInTune ? "#3fae6b" : "#eef5f0";
  const statusColor  = isInTune ? "#3fae6b" : isFlat ? "#4f9dde" : "#e0b341";
  const statusLabel  = !note ? "—" : isInTune ? "OK" : isFlat ? "♭ Baixo" : "♯ Alto";
  const glowColor    = isInTune ? "rgba(63,174,107,0.35)" : "rgba(255,255,255,0.08)";

  // ── Render ───────────────────────────────────────────────
  return (
    <div style={{ maxWidth: 460, margin: "0 auto", padding: "24px 18px 100px", fontFamily: "'Montserrat',sans-serif", userSelect: "none" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 28 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <svg width="24" height="28" viewBox="0 0 24 28" fill="none">
            <path d="M12 2C8.5 2 6 5 6 8.5C6 12 9 15.5 12 17C15 15.5 18 12 18 8.5C18 5 15.5 2 12 2Z" fill="#e0b341"/>
            <path d="M12 17L10.5 26H13.5L12 17Z" fill="#e0b341" opacity=".7"/>
          </svg>
          <span style={{ fontWeight: 800, fontSize: 20, color: "#eef5f0", letterSpacing: "-0.3px" }}>Afinador</span>
        </div>
        {/* Toggle instrumento */}
        <div style={{ display: "inline-flex", background: "#0c2419", border: "1px solid #1d4435", borderRadius: 10, padding: 3, gap: 2 }}>
          {[["guitar","Violão"],["bass","Baixo"]].map(([k,lbl]) => (
            <button key={k} onClick={() => { setInstrument(k); setSelString(null); }}
              style={{ padding: "5px 13px", borderRadius: 7, border: "none", cursor: "pointer", fontSize: 12, fontWeight: 700,
                fontFamily: "'Montserrat',sans-serif",
                background: instrument === k ? "linear-gradient(135deg,#1d6b45,#0f4a30)" : "transparent",
                color: instrument === k ? "#fff" : "#6fae8a" }}>
              {lbl}
            </button>
          ))}
        </div>
      ...
      {/* Mostrador circular */}
      <div style={{ position: "relative", width: "100%", maxWidth: 340, margin: "0 auto 8px", aspectRatio: "1/0.62" }}>
        <svg viewBox="0 0 340 210" style={{ width: "100%", height: "auto", overflow: "visible" }}>
          {/* Arco de fundo */}
          <path d="M 30 190 A 140 140 0 0 1 310 190" fill="none" stroke="#0c2419" strokeWidth="2"/>
          {/* Arco iluminado (glow quando afinado) */}
          <path d="M 30 190 A 140 140 0 0 1 310 190" fill="none"
            stroke={isInTune ? "#3fae6b" : "#1d4435"} strokeWidth={isInTune ? 3 : 1.5}
            style={{ transition: "stroke .25s, stroke-width .25s", filter: isInTune ? "drop-shadow(0 0 6px rgba(63,174,107,0.7))" : "none" }}/>

          {/* Marcações de cents: –50, –25, 0, +25, +50 */}
          {[-50,-25,0,25,50].map(c => {
            const a = (c / 60) * 80 * Math.PI / 180;
            const cx = 170 + 130 * Math.sin(a);
            const cy = 190 - 130 * Math.cos(a);
            const cx2 = 170 + 145 * Math.sin(a);
            const cy2 = 190 - 145 * Math.cos(a);
            const isCenter = c === 0;
            return (
              <g key={c}>
                <line x1={cx} y1={cy} x2={cx2} y2={cy2}
                  stroke={isCenter ? "#3fae6b" : "#1d4435"}
                  strokeWidth={isCenter ? 2 : 1}/>
                {Math.abs(c) === 50 && (
                  <text x={cx2 + (c < 0 ? -8 : 8)} y={cy2 + 4}
                    fill="#2f5a40" fontSize="11" fontFamily="Montserrat" textAnchor="middle">{c}</text>
                )}
              </g>
            );
          })}

          {/* Palheta indicadora no topo */}
          <g transform={`rotate(${angle}, 170, 190)`} style={{ transition: "transform 0.08s linear" }}>
            <polygon points="170,58 167,65 173,65" fill={needleColor}
              style={{ filter: isInTune ? "drop-shadow(0 0 4px rgba(63,174,107,0.9))" : "none", transition: "fill .2s" }}/>
          </g>

          {/* Ponteiro */}
          <g transform={`rotate(${angle}, 170, 190)`} style={{ transition: "transform 0.08s linear" }}>
            <line x1="170" y1="70" x2="170" y2="185"
              stroke={needleColor} strokeWidth="2.5" strokeLinecap="round"
              style={{ filter: isInTune ? "drop-shadow(0 0 6px rgba(63,174,107,0.8))" : "none", transition: "stroke .2s" }}/>
          </g>

          {/* Pivô */}
          <circle cx="170" cy="190" r="9" fill={needleColor}
            style={{ filter: `drop-shadow(0 0 8px ${glowColor})`, transition: "fill .2s, filter .2s" }}/>
        </svg>

        {/* Nota + freq no centro do mostrador */}
        <div style={{ position: "absolute", left: "50%", top: "56%", transform: "translate(-50%,-50%)", textAlign: "center", pointerEvents: "none" }}>
          <div style={{ fontSize: "clamp(36px,10vw,52px)", fontWeight: 900, lineHeight: 1, color: note ? "#eef5f0" : "#1d4435",
            transition: "color .15s" }}>
            {note ? note.name : "—"}
          </div>
          {note && (
            <div style={{ fontSize: 12, color: "#6fae8a", marginTop: 2, fontVariantNumeric: "tabular-nums" }}>
      ...
      <div style={{ textAlign: "center", marginBottom: 28 }}>
        <div style={{ fontSize: "clamp(22px,7vw,32px)", fontWeight: 900, color: statusColor,
          transition: "color .25s", letterSpacing: "-0.5px" }}>
          {active ? statusLabel : "Toque para ligar"}
        </div>
        {note && !isInTune && (
          <div style={{ fontSize: 13, color: "#6fae8a", marginTop: 4 }}>
            {Math.abs(cents)} cents {isFlat ? "abaixo" : "acima"}
          </div>
        )}
      </div>
      ...
      <div style={{ display: "flex", justifyContent: "center", marginBottom: 32 }}>
        <button onClick={toggle} style={{
          ...
          background: active
            ? "linear-gradient(135deg,#0f4a30,#0a3422)"
            : "linear-gradient(135deg,#fff,#dff0e6)",
          color: active ? "#3fae6b" : "#0d3d28",
          display: "flex", alignItems: "center", justifyContent: "center",
          boxShadow: active ? "0 0 0 3px rgba(63,174,107,.4), 0 8px 24px rgba(0,0,0,.4)" : "0 8px 24px rgba(255,255,255,.15)",
          transition: "all .2s",
        }}>
          {active ? (
            // Stop icon
      ...
      {/* Erro de permissão */}
      {error && (
        <div style={{ background: "rgba(232,85,77,.12)", border: "1px solid #e8554d44", borderRadius: 12, padding: "12px 16px",
          color: "#e8554d", fontSize: 13, textAlign: "center", marginBottom: 20 }}>
          {error}
        </div>
      )}

      {/* Cordas */}
      <div style={{ background: "#0c2419", border: "1px solid #15392b", borderRadius: 16, padding: "14px 12px" }}>
        <div style={{ fontSize: 11, color: "#3fae6b", fontWeight: 700, textTransform: "uppercase", letterSpacing: ".08em",
          marginBottom: 12, textAlign: "center" }}>
          {instrument === "bass" ? "Cordas do baixo" : "Cordas do violão"}
        </div>
        <div style={{ display: "flex", justifyContent: "center", gap: instrument === "bass" ? 14 : 10 }}>
          {strings.map((s, i) => {
            const isActive  = activeIdx === i;
            const isSel     = selString === i;
            return (
              <div key={i} onClick={() => setSelString(isSel ? null : i)}
                style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 6, cursor: "pointer" }}>
                {/* Indicador de acordo (pontinho acima da corda ativa) */}
                <div style={{
                  width: 6, height: 6, borderRadius: "50%",
                  background: isActive && note ? (isInTune ? "#3fae6b" : isFlat ? "#4f9dde" : "#e0b341") : "transparent",
                  transition: "background .2s",
                }}/>
                {/* Círculo da corda */}
                <div style={{
                  width: 44, height: 44, borderRadius: "50%",
                  border: `2px solid ${isSel ? "#3fae6b" : isActive && note ? (isInTune ? "#3fae6b66" : "#1d4435") : "#1d4435"}`,
                  background: isSel ? "rgba(63,174,107,.2)" : isActive && note ? "rgba(63,174,107,.08)" : "#08160f",
                  display: "flex", alignItems: "center", justifyContent: "center",
                  transition: "all .15s",
                }}>
                  <span style={{ fontWeight: 800, fontSize: 16, color: isSel ? "#3fae6b" : isActive && note ? "#eef5f0" : "#3a6b50" }}>
                    {s.display}
                  </span>
                </div>
                {/* Frequência alvo */}
                <div style={{ fontSize: 9, color: "#3a6b50", fontVariantNumeric: "tabular-nums" }}>
                  {s.freq < 100 ? s.freq.toFixed(1) : Math.round(s.freq)} Hz
                </div>
              </div>
            );
          })}
      ...
      {/* Dica de uso */}
      {!active && !error && (
        <div style={{ marginTop: 20, textAlign: "center", fontSize: 12, color: "#3a6b50", lineHeight: 1.6 }}>
          Toque no botão acima para ativar o microfone.<br/>
          Toque em uma corda para fixar o alvo de afinação.
        </div>
      )}
    </div>
  );
    ...
    <div style={{ minHeight: "100vh", background: "linear-gradient(165deg,#0a1f17 0%,#08160f 55%,#06110b 100%)", color: "#eef5f0", fontFamily: "'Montserrat',sans-serif" }}>
      ...
      {showTuner && (
        <div style={{
          ...
          background: "linear-gradient(180deg,#08160f 0%,#0a1f17 100%)",
          overflowY: "auto",
          ...
          <div style={{ display: "flex", justifyContent: "flex-end", padding: "14px 18px 0" }}>
            <button onClick={() => setShowTuner(false)} style={{
              background: "rgba(255,255,255,.05)", border: "1px solid #1d4435",
              borderRadius: 10, color: "#eef5f0", width: 36, height: 36,
      ...
      {/* ── Barra de aba fixa — Afinador ─────────────────────── */}
      <div style={{
        position: "fixed", bottom: 0, left: 0, right: 0, zIndex: 400,
        background: "rgba(8,22,15,0.97)",
        borderTop: "1px solid #15392b",
        display: "flex",
        ...
        {/* Aba Afinador */}
        <button onClick={() => setShowTuner(v => !v)} style={{
          flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center",
          gap: 4, padding: "10px 0 8px", border: "none", cursor: "pointer",
          background: showTuner ? "rgba(63,174,107,.12)" : "transparent",
          borderTop: showTuner ? "2px solid #3fae6b" : "2px solid transparent",
          transition: "all .15s",
        }}>
          {/* Ícone palheta */}
          <svg width="22" height="26" viewBox="0 0 24 28" fill="none">
            <path d="M12 2C8.5 2 6 5 6 8.5C6 12 9 15.5 12 17C15 15.5 18 12 18 8.5C18 5 15.5 2 12 2Z"
              fill={showTuner ? "#3fae6b" : "#4a7a5f"}/>
            <path d="M12 17L10.5 26H13.5L12 17Z"
              fill={showTuner ? "#3fae6b" : "#4a7a5f"} opacity=".7"/>
          </svg>
          <span style={{ fontSize: 10, fontWeight: 700, color: showTuner ? "#3fae6b" : "#4a7a5f",
            fontFamily: "'Montserrat',sans-serif", textTransform: "uppercase", letterSpacing: ".06em" }}>
            Afinador
          </span>
        </button>
      </div>
